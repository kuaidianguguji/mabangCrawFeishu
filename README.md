...
| `timing` | 元素、页面、登录和采集等待时间，提交登录后的额外确认时间及采集重试次数 |
...
1. 使用固定浏览器配置及 `profile_dir` 打开首页。存在 `//a[@id="login-btn"]` 时点击登录入口，等待账号、密码输入框后提交；检测到 `//div[@id="mb-user"]` 才确认登录成功。验证码可在登录等待时间内手动完成，日志会打印 `login_timeout`、`post_submit_timeout` 和实际成功标志 XPath。
2. 登录成功后，先启动接口监听，再进入云 BI。轮询 `//section//div[@data-filter="timezone"]//button` 的内容并输出日志；内容不含 `UTC` 时继续等待渲染，不将内部编码当成时区。提交登录后还会按 `timing.post_submit_timeout` 单独等待 `//div[@id="mb-user"]`，避免登录回调较慢时提前关闭浏览器。
...
| `capture_diagnostics.json` | 每轮监听到的 XHR/Fetch 数量、目标包数量、HTTP/JSON 拒绝原因、非目标接口示例及缺失模块；不保存请求头和响应业务正文 |
...
| Network 有 JSON，但程序提示缺少接口 | 查看 `capture_diagnostics.json`：目标包为 0 表示没匹配到目标响应；目标包存在但 `rejected` 大于 0 表示 HTTP/JSON 校验未通过。程序监听当前标签页的全部 XHR/Fetch，再按配置路径或 `/modules/模块名` 识别，兼容接口域名及网关前缀变化；若仍为 0，核对请求是否来自另一个标签页、iframe 或不同接口路径 |
| 整体重试时浏览器关闭后重开 | 获取数据前尝试失败时，程序主动关闭专用浏览器并重新启动；日志会明确标注主动关闭原因，不能仅据此判断为浏览器崩溃 |

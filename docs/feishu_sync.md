...
正常返回少于五名时，旧排名多余位置会清空；结构错误时跳过该表并记录错误，不清空旧表。未知平台留在本地，不把未返回平台填零。不同表不保证原子写入，失败后依据报告重跑；每次都会重新读取记录。

飞书临时请求失败按 `retry.feishu.count` / `interval` 重试；后续整体流程失败按 `retry.after_capture.count` / `interval` 复用已获取 JSON 重试。新增记录的同一目标与载荷固定使用同一 `client_token`，请求重试和整体重跑均保持一致；整体重跑前会重新读取线上记录，已写入且无变化的记录跳过。字段、权限和参数错误需要修正后再运行，不自动反复提交。所有次数均为首次执行之外的额外次数，间隔单位为秒。
